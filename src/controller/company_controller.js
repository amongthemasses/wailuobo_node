...
class CompanyController {
    ...
    async getCompany(ctx) {
        ...
        let query = `
            SELECT a.*,
                   GROUP_CONCAT(bt.id, '|,|', bt.text SEPARATOR '|.|')      AS texts,
                   GROUP_CONCAT(ct.id, '|,|', ct.main_text SEPARATOR '|.|') AS main_texts,
            FROM company AS a
                     LEFT JOIN company_text AS bt ON a.id = bt.company_id
                     LEFT JOIN company_main AS ct ON a.id = ct.company_id
        ...
        try {
            ...
            let reList = result.map((item, i) => {
                ...
                if (String(item.main_texts).trim()) {
            ...
            return ctx.body = {code: ResponseCode.missingParameter, message: error.message, error};
        }
    }
    ...
    async insertCompany(ctx) {
        let {
            baseId, phoneNumber, post, company, startDate, endDate, address, texts, mainTexts
        } = ctx.request.body || {};
        ...
        if (!post || (typeof post != "string")) {
            return ctx.body = {code: ResponseCode.missingParameter, message: "missing parameter 'post'"};
        }
        if (!company || (typeof company != "string")) {
            return ctx.body = {code: ResponseCode.missingParameter, message: "missing parameter 'company'"};
        }
        if (!startDate || (typeof startDate != "number")) {
            return ctx.body = {code: ResponseCode.missingParameter, message: "missing parameter 'startDate'"};
        }
        if (!endDate || (typeof endDate != "number")) {
            return ctx.body = {code: ResponseCode.missingParameter, message: "missing parameter 'endDate'"};
        }
        ...
        try {
            let query = `
                INSERT INTO company (base_id, phone_number, post, company, start_date, end_date, address, update_date,
                                     create_date)
                VALUES (${baseId}, '${phoneNumber}', '${post}', '${company}', ${startDate}, ${endDate}, '${address}',
                        NOW(), NOW())
            `;
            ...
            conn.commit();
            return ctx.body = {code: ResponseCode.success, data: {insertId}, message: "创建成功！"};
        } catch (error) {
            await conn.rollback();
            return ctx.body = {code: ResponseCode.missingParameter, message: error.message, error};
        }
    ...
    async updateCompany(ctx) {
        let {
            companyId, phoneNumber, post, company, startDate, endDate, address, texts, mainTexts
        } = ctx.request.body || {};
